...
class QueryUtils {
    ...
    */
    async getAssetHistory(name, owner) {

        let ledgerKey = await this.ctx.stub.createCompositeKey(this.name, [owner, name]);
    ...
    * 可以用 name 和 owner 分别来查询
    */
    async queryKeyByPartial(assets) {

        if (arguments.length < 1) {
            // throw new Error('Incorrect number of arguments. Expecting 1');
            assets = [];
        }else {
            assets = DataAsset.splitKey(assets);
        }
        ...
        const resultsIterator = await this.ctx.stub.getStateByPartialCompositeKey(this.name, assets);
    ...
    * @param {String} publisher data asset publisher
    */
    async queryKeyByPublisher(publisher) {
        //  
        let self = this;
        if (arguments.length < 1) {
        ...
        queryString.selector.publisher = publisher;
    ...
    /*
     * {values} json 放置 selector 的属性
     */

    async queryKeyByValues(values) {
        //  
        let self = this;
        if (arguments.length < 1) {
            throw new Error('Incorrect number of arguments. Expecting one of values to query.');
        }
        let queryString = {};
        queryString.selector = values;
                    ...
                    if (res.value.is_delete) {
                        ...
                        try {
                            ...
                            switch (jsonRes.Value.currentState) {
                                case 1:
                                    jsonRes.Value.currentState = 'PUSHED';
                                    break;
                                case 2:
                                    jsonRes.Value.currentState = 'CHECK';
                                    break;
                                case 3:
                                    jsonRes.Value.currentState = 'UPDATE';
                                    break;
                                case 4:
                                    jsonRes.Value.currentState = 'OVERDUE';
                                    break;
...
// console.log(DataAsset.splitKey("1asf"));
// console.log(DataAsset.splitKey("123:12"));
