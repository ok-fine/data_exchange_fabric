...
class QueryUtils {
    ...
    */
    async getRoleHistory(org, name) {

        let ledgerKey = await this.ctx.stub.createCompositeKey(this.name, [org, name]);
    ...
    */
    ...
    // 暂时还无法使用
    async queryKeyByRange(startKey, endKey){
        // let resultsIterator = null;
        if (arguments.length === 0) {
            console.log('Quer all roles in the channel.');
            startKey = '';
            endKey = '';
            // resultsIterator = await this.ctx.stub.getStateByRange('', '');
        }
        startKey = await this.ctx.stub.createCompositeKey(this.name, [startKey]);
        endKey = await this.ctx.stub.createCompositeKey(this.name, [endKey]);
        console.log('prefix: ' + startKey + "   " + endKey);
