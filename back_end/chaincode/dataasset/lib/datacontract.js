...
class DataAssetContract extends Contract {
    ...
    async exist(ctx, name, params) {
        ...
        if(data[0] === undefined){
            console.log(`${dataKey} is not exist`);
            return false;
        }
 ...
 //     */
    ...
    async searchByOwner(ctx, roleKey, publicPem, org, user) {
        ...
        for(var d of data){
            let cando = await this.canDo(ctx, roleKey, `${d.Record.name}:${d.Record.owner}`, 'searchByOwner', publicPem);
            if(cando){
                res.push(d);
            }
        }
    ...
    async searchByOrg(ctx, roleKey, publicPem, org) {
        ...
        for(var d of data){
            let cando = await this.canDo(ctx, roleKey, `${d.Record.name}:${d.Record.owner}`, 'searchByOrgr', publicPem);
            if(cando){
                res.push(d);
            }
        }
    ...
    async searchByName(ctx, roleKey, publicPem, name) {
        ...
        for(var d of data){
            let cando = await this.canDo(ctx, roleKey, `${d.Record.name}:${d.Record.owner}`, 'searchByName', publicPem);
            if(cando){
                res.push(d);
            }
        }
    ...
    async searchAll(ctx, roleKey, publicPem) {
        ...
        for(var d of data){
            let cando = await this.canDo(ctx, roleKey, `${d.Record.name}:${d.Record.owner}`, 'searchAll', publicPem);
            if(cando){
                res.push(d);
            }
        }
    ...
    async canDo(ctx, roleKey, dataKey, func, publicPem){
        console.log('cando: ' + roleKey + ' ' + dataKey);
        let channel = ctx.stub.getChannelID();
        ...
        if( resCrypto.payload === false){
            console.log(`角色 ${roleKey} 不存在`);
            return false;
        }
